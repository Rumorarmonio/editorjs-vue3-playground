...
- Активный scope по `SPEC.md`: media gallery / slider block после завершения custom inline tools.
...
- Проект переведён к следующему крупному этапу по `SPEC.md`: media gallery / slider block.
...
- Полноценный media workflow с backend/upload storage остаётся поздним этапом; текущий media gallery / slider этап должен использовать URL, public/static assets и при необходимости временные data/blob URLs.
...
Следующий шаг: проанализировать текущие field helpers, rich fields, renderer mapping и image/embed contracts перед проектированием минимальной data schema для media gallery / slider block.
