...
- Открытый вопрос активного accessibility-этапа: насколько глубоко дорабатывать keyboard access для `@editorjs/table` controls. Нужно отдельно проверить нативное перемещение по ячейкам `Tab` / `Shift+Tab`, поведение стрелок внутри/между ячейками и возможность безопасно добавить tab stops к Table Tool add-column / row-column tune controls без форка инструмента.
...
Следующий шаг: вручную проверить tab-order верхней панели editor page после возврата фокуса из Editor.js, кликабельность toggle label/track, focus на select в `Notice`/`MediaGallery`, enabled-кнопки media cards, nested editor scenarios после scoped `contenteditable`-правки и keyboard-поведение Table Tool.
