...
- Активный scope по `SPEC.md`: расширение набора контентных блоков и plugins; после проверки `CTA/Button` и `Code snippet` решено добавить `Raw HTML` в этом же этапе.
...
- Этап расширения набора контентных блоков и plugins снова активен: typed custom blocks `cta` и `codeSnippet` добавлены и проверены, следующим выбран `Raw HTML`; link preview и `Warning` не добавляются.
...
- Этап расширения набора контентных блоков и plugins частично выполнен: добавлены shared-типы, normalizers/guards, registry entries, validation, localized editor UI, tools `CtaTool` / `CodeSnippetTool`, renderer components `EditorCtaBlock` / `EditorCodeSnippetBlock`, theme-aware admin/renderer styles и demo blocks в `content/default-page.json`. Новые blocks подключены только к основному editor; nested editor whitelist'ы не расширялись. `npm run check` проходит с существующими предупреждениями `vue/no-v-html`; `npm run build` проходит.
...
- Ручная проверка подтвердила работу `CTA/Button` и `Code snippet`; следующий выбранный block этого этапа — `Raw HTML`.
...
Следующий шаг: спроектировать и реализовать `Raw HTML` block как trusted/admin-only escape hatch: data contract, editor textarea UI, sanitizer/allowlist, renderer behavior, validation, i18n, theme/accessibility states, demo content и smoke-check save/load/render/import/export/reset.
