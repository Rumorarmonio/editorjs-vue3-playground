...
Следующий шаг: вручную проверить editor canvas после выноса admin skin в глобальный stylesheet, а также preview sidebar в режимах `По заголовкам` и `По якорям и меткам`, language dropdown, block reveal animations и embed Fancybox/inline modes после `Reset draft`.
