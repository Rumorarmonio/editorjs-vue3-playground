...
- Создан базовый доменный каркас: OutputData-compatible типы, типы стандартных блоков, registry-основа, `content/default-page.json` и draft-source helpers без подключения Editor.js runtime.
- Editor.js, кастомные блоки, renderer и UI draft workflow пока не реализованы.
...
- Draft-source guard теперь принимает из `localStorage` только OutputData-like JSON с block types, известными текущему registry; глубокая проверка `data` остаётся поздним validation-этапом.
...
Следующий крупный шаг: реализовать editor shell и preview shell, подключить загрузку данных с приоритетом draft из `localStorage` -> `content/default-page.json`, пока без подключения стандартных Editor.js tools.
