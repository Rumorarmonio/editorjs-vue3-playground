...
- Активный scope по `SPEC.md`: Import JSON после завершения sidebar navigation из JSON.
...
- Проект переведён к следующему крупному этапу по `SPEC.md`: Import JSON.
...
- Проект переведён к этапу Import JSON: активный план сфокусирован на ручном import JSON, базовой проверке структуры и записи валидного imported content в local draft.
...
Следующий шаг: проанализировать текущий `useEditorContentSource`, editor save/load workflow и preview actions перед реализацией ручного Import JSON.
