...
- Активный scope по `SPEC.md`: composite blocks / `TwoColumns` после завершения первого nested editor scenario и reusable rich fields.
...
- Проект переведён к этапу composite blocks / `TwoColumns`: следующий фокус — первый custom block с двумя независимыми nested editor containers.
...
- Проект переведён к этапу composite blocks / `TwoColumns`: активный план сфокусирован на JSON-контракте, двух nested editor outputs, renderer, guard/normalization и проверке многоконтейнерного lifecycle.
...
- Для текущего этапа нужно определить минимальный способ управлять двумя nested editor instances внутри `TwoColumns` без преждевременного общего composite block manager.
...
Следующий шаг: спроектировать минимальный JSON-контракт `TwoColumns` и допустимый whitelist tools для левой и правой nested editor columns.
