...
- Embed Fancybox display реализован как четвёртое optional improvement: `EmbedDisplayTune` подключён только к `embed`, общий whitelist embed-сервисов расширен Rutube и VK Video, preview renderer поддерживает inline iframe и Fancybox iframe modes, а `content/default-page.json` содержит inline/Fancybox examples для YouTube/Vimeo/Coub/Rutube/VK Video.
...
Следующий шаг: вручную проверить editor canvas после выноса admin skin в глобальный stylesheet, а также preview sidebar в режимах `По заголовкам` и `По якорям и меткам`, language dropdown, block reveal animations, embed Fancybox/inline modes и смену URL созданного embed-блока после `Reset draft`.
