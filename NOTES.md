...
- Проект переведён к этапу Masks: ближайший фокус — зафиксировать, что в текущих доменных блоках реальный кандидат только slug-like `galleryId`, затем добавить отдельный demo-only custom block для проверки phone/date/time/price/card/email masks и решить, нужна ли зависимость `imask`.
...
Следующий шаг: добавить demo-only custom block для проверки phone/date/time/price/card/email masks, затем решить, нужна ли зависимость `imask` или достаточно локальной slug-like нормализации для `galleryId`.
