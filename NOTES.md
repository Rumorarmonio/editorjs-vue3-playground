...
- Активный scope по `SPEC.md`: расширение набора контентных блоков и plugins; этап Некритичные улучшения временно закрыт после серии optional improvements и будет продолжен после следующего этапа.
...
- Этап расширения набора контентных блоков и plugins активирован. Он должен начаться с research/decision по custom `CTA/Button`, `Raw HTML`, code block, link preview и `Warning`, а не с автоматического добавления всех найденных пакетов.
...
Следующий шаг: начать активный этап расширения набора контентных блоков и plugins с research/decision по custom `CTA/Button`, `Raw HTML`, code block, link preview и `Warning`. Перед первой реализацией вручную проверить, что поздние optional improvements не сломали editor canvas, preview sidebar, language dropdown, block reveal animations, embed Fancybox/inline modes и смену URL созданного embed-блока после `Reset draft`.
