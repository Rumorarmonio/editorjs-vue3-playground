...
- Активный scope по `SPEC.md`: reusable rich fields после первого успешного nested editor scenario.
...
- Проект переведён к этапу reusable rich fields; следующий шаг должен вынести проверенный nested editor lifecycle в `RichParagraphField` и `RichHeaderField` без изменения публичной схемы данных `SectionIntro`.
...
- Reusable `RichParagraphField` / `RichHeaderField` являются текущим активным этапом после проверки первого nested editor lifecycle.
...
- На текущем этапе нужно решить, насколько обобщать существующий helper: достаточно минимального reusable field lifecycle для `RichParagraphField` / `RichHeaderField`, без преждевременного перехода к composite block manager.
...
Следующий шаг: изучить текущие `createNestedParagraphEditor` и `SectionIntroTool`, затем спроектировать минимальный reusable lifecycle для `RichParagraphField` и `RichHeaderField`.
