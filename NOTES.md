...
- Активный scope по `SPEC.md`: Некритичные улучшения; предыдущий этап Клавиатурная навигация и accessibility polish завершён.
...
- После обсуждения дополнительных Editor.js plugins запланирован следующий крупный этап: расширение набора контентных блоков и plugins. Он должен начаться с research/decision по custom `CTA/Button`, `Raw HTML`, code block, link preview и `Warning`, а не с автоматического добавления всех найденных пакетов.
...
- Этап Некритичные улучшения пока не завершён: перед переходом к следующему крупному этапу расширения контентных блоков/plugins нужно выбрать и реализовать один или несколько небольших optional improvements, затем проверить, что они не ломают save/load, Import JSON, validation, masks, localization, theme, keyboard scenarios, preview, `Reset draft` и `Export JSON`.
...
Следующий шаг: вручную проверить editor canvas после выноса admin skin в глобальный stylesheet, а также preview sidebar в режимах `По заголовкам` и `По якорям и меткам`, language dropdown, block reveal animations, embed Fancybox/inline modes и смену URL созданного embed-блока после `Reset draft`. После закрытия текущего этапа перейти к research/decision этапу по расширению набора контентных блоков и plugins.
