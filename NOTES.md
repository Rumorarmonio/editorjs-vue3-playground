...
- Активный scope по `SPEC.md`: Некритичные улучшения; этап расширения набора контентных блоков и plugins завершён после добавления `CTA/Button`, `Code snippet` и `Raw HTML`.
...
- Этап расширения набора контентных блоков и plugins завершён: typed custom blocks `cta`, `codeSnippet` и `rawHtml` добавлены; link preview и `Warning` не добавляются.
...
- `Raw HTML` добавляется в текущем этапе только как trusted/admin-only block с отдельным security/sanitizer/renderer contract; произвольный JavaScript execution из content JSON должен оставаться запрещённым.
- `Raw HTML` renderer использует allowlist sanitizer: scripts, iframes, event handlers, inline styles и unsafe attributes удаляются; block остаётся trusted/admin-only escape hatch, а не общей HTML execution зоной.
...
- Этап Некритичные улучшения временно закрыт после серии optional improvements: language switcher, heading-based sidebar navigation, block reveal animations, embed Fancybox display, редактирование URL существующего embed-блока, декомпозиция renderer components и вынос admin editor skin в глобальный stylesheet. После реализации следующего этапа к нему планируется вернуться, потому что новые улучшения появляются постоянно.
...
- Review-fixes для расширения blocks/plugins внесены: Raw HTML sanitizer удаляет запрещённые элементы целиком и нормализует root-relative links через Nuxt `baseURL`; CTA event name больше не переписывается silently, а валидируется как введённый идентификатор; общий CTA renderer только dispatches `editor:cta-action`, а demo modal перенесён в preview shell; `CtaTool` скрывает нерелевантные поля для link/event action types. `npm run check` и `npm run build` проходят.
- Ручная проверка подтвердила работу `CTA/Button` и `Code snippet`; `Raw HTML`, CTA event action и syntax highlighting реализованы и требуют ближайшего browser smoke-check.
...
Следующий шаг: вручную проверить `Raw HTML`, CTA event action и syntax highlighting в editor/preview: create/edit/save/reload, Import JSON, Export JSON, Reset draft, light/dark themes, полное удаление `<script>`/unsafe elements, нормализацию root-relative ссылок в Raw HTML и demo modal для `open-demo-modal`. После smoke-check продолжить активный этап Некритичные улучшения.
