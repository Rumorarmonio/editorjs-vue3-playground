...
Следующий шаг: вручную проверить `Raw HTML`, CTA event action, nested CTA и syntax highlighting в editor/preview: create/edit/save/reload, Import JSON, Export JSON, Reset draft, light/dark themes, unsafe rendering со стилями и demo alert script, переключение `rawHtmlRenderMode` в `safe` для проверки sanitizer и demo modal для `open-demo-modal`. После smoke-check продолжить активный этап Некритичные улучшения.
