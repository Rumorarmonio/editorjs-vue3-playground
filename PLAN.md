...
3. Реализовывать улучшения по одному, с узкими изменениями и проверкой связанных сценариев — в работе; первый срез language switcher выполнен.
4. После каждого значимого изменения запускать соразмерные проверки — выполнено для первого среза через `npm run check` и `npm run build`.
